...
export const getLengthOfCategoryPosts = async (req, res) => {
  try {
    ...
    posts.forEach((post) => {
      ...
      if (post.categories === "mountains") {
        categories.mountains.push(post);
      }
    });
    ...
    res.status(200).json({
      ...
      mountains: categories.mountains.length,
    });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
...
export const topCategoryPosts = async (req, res) => {
  try {
    ...
    res.status(200).json({
      ...
      // mountains: [...posts.mountains][0] || {},
      success_stories: [...posts.success_stories][0] || {},
      famous_places: [...posts.famous_places][0] || {},
    });
  } catch (error) {
